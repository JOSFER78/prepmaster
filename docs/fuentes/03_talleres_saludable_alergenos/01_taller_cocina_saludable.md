...
```mermaid
flowchart LR
    A[Cromatografía Fitoquímica] --> B[Rojo: Licopeno & Antocianinas]
    A --> C[Naranja/Amarillo: Carotenoides & Luteína]
    A --> D[Verde: Clorofila, Glucosinolatos & Sulforafano]
    A --> E[Morado/Azul: Resveratrol & Polifenoles]
    A --> F[Blanco: Alicina & Quercetina]
```
...
```mermaid
graph LR
    A[Alimento Ingerido] --> B{Densidad Nutricional}
    B -->|Alta: Micronutrientes/Kcal| C[Homeostasis Metabólica & Saciedad]
    B -->|Baja: Calorías Vacías| D[Inflamación de Bajo Grado & Resistencia Insulina]
    
    C --> E[Control de Curva Glucémica: IG vs CG]
    E --> F[Almidón Retrógrado RS3]
    F --> G[Fermentación en Colon Derecho]
    G --> H[Síntesis AGCC: Butirato, Acetato, Propionato]
    H --> I[Integridad de Barrera Intestinal & Eje Cerebro]
```
...
```mermaid
graph TD
    A[Fibra Prebiótica & RS3] --> B[Microbiota Colónica Anaerobia]
    B --> C[Butirato / Ácido Butírico]
    B --> D[Propionato / Ácido Propiónico]
    B --> E[Acetato / Ácido Acético]
    
    C --> C1[Nutriente primario de Colonocitos 70% energía]
    C --> C2[Expresión de Claudinas & Ocludinas: Cierre de Barrera]
    C --> C3[Inducción de Linfocitos Treg Antiinflamatorios]
    
    D --> D1[Transporte Portal al Hígado: Inhibición de Gluconeogénesis]
    D --> D2[Modulación de Saciedad a nivel Hipotalámico GLP-1 / PYY]
    
    E --> E1[Sustrato para Síntesis de Colesterol HDL y Músculo Periférico]
    E --> E2[Acidificación del Lumen pH 5.5: Bloqueo de Patógenos]
```
...
```mermaid
graph TD
    A[Tratamiento Térmico] --> B[Cocciones Hidrotérmicas & Suaves: 55°C - 100°C]
    A --> C[Cocciones Secas & Extremas: > 160°C - 220°C]
    
    B --> B1[Vapor: Retención 90% Vitamina C & Fitoquímicos]
    B --> B2[Papillote: Cero Oxidación Lipídica & Jugos Intactos]
    B --> B3[Sous-Vide: Desnaturalización Precisa & Nula Pérdida de Humedad]
    B --> B4[Wok Rápido: Sellado Flash < 2 min sin Degeneración]
    
    C --> C1[Fritura Reutilizada: Acroleína & Peróxidos Lipídicos]
    C --> C2[Plancha Sobrecalentada: HCA & HAP por Pirolisis]
    C --> C3[Horno Fuerte sin Humedad: AGEs & Acrilamida]
```
...
```mermaid
flowchart TD
    A[Bases Emulsionantes Nobles] --> B[Tahini Tostado + Limón + Ajo Asado]
    A --> C[Yogur Griego / Kéfir + Eneldo + Pepino]
    A --> D[Aguacate Hass + Cilantro + Lima]
    A --> E[Mostaza Antigua D.O.P. + AOVE + Vinagre Manzana]
    A --> F[Passata de Tomate Reducida con Verduras Dulces]
```
...
```mermaid
graph TD
    A[Lomos de Salmón Salvaje Limpios] --> B[Marinado 10 min con Limón, Eneldo y AOVE]
    C[Hinojo, Calabacín y Puerro en Juliana Fina] --> D[Salteado Rápido 2 min con Sal Marina]
    
    E[Quinoa Tricolor Lavada] --> F[Cocción 14 min en Caldo Vegetal Ligero]
    
    B --> G[Montaje en Paquetes de Papel Sulfurizado: Cama Vegetal + Salmón + Rodaja Naranja]
    D --> G
    G --> H[Horneado Sellado a 170°C durante 12-14 min]
    H --> I[Apertura de Papillote + Emulsión de AOVE al Eneldo + Servir sobre Quinoa]
    F --> I
```
...
```mermaid
graph TD
    A[Pechugas de Pollo Campero] --> B[Bolsa de Vacío con Cúrcuma, Jengibre, Ajo y AOVE]
    B --> C[Baño María Sous-Vide a 63.5°C por 60 min]
    C --> D[Marcado Rápido en Sartén 45 seg por lado]
    
    E[Dados de Boniato con Romero y Sal] --> F[Horno a 185°C por 25 min]
    F --> G[Enfriado a 4°C por 24h para generar RS3]
    
    H[Tallos de Bimi Fresco] --> I[Vapor 4 min: Verde Brillante al Dente]
    
    D --> J[Ensamble: Suprema Fileteada + Boniato Templado + Bimi + Salsa Tahini-Limón]
    G --> J
    I --> J
```
...
```mermaid
graph TD
    A[Arroz Integral de Grano Redondo] --> B[Cocción 35 min + Refrigeración 24h a 3°C: Almidón RS3]
    C[Lentejas Pardinas Secas] --> D[Cocción con Laurel y Hortalizas 25 min]
    
    E[Calabaza Cacahuete en Dados] --> F[Asado a 190°C con Tomillo y AOVE 22 min]
    
    G[Vinagreta de Mostaza Antigua, AOVE y Vinagre de Manzana] --> H[Emulsión en Frasco]
    
    B --> I[Montaje Bowl Harvard: Base Arroz RS3 + Lentejas + Calabaza Asada + Espinaca Baby + Aguacate]
    D --> I
    F --> I
    H --> I
```
...
```mermaid
graph TD
    A[Bloque de Tofu Firme] --> B[Prensado Mecánico 25 min: Extracción Agua Libre]
    B --> C[Marinado con Tamari GF, Jengibre y Ajo]
    C --> D[Rebozado en Sésamo Tostado y Almidón de Tapioca]
    D --> E[Horneado a 200°C por 20 min: Costra Crujiente]
    
    F[Trigo Sarraceno Tostado - Kasha] --> G[Cocción en Caldo 12 min]
    
    H[Pak Choi, Pimiento Rojo, Setas Shiitake] --> I[Salteado Rápido al Wok con AOVE a Fuego Vivo 3 min]
    
    E --> J[Ensamble Wok: Trigo Sarraceno + Vegetales Salteados + Dados de Tofu Crujiente]
    G --> J
    I --> J
```
...
```mermaid
graph TD
    A[Plato Cocinado a > 75°C] --> B[Abatimiento Térmico Inverso: Baño María Hielo]
    B --> C{Temperatura < 10°C en < 60 min}
    C --> D[Envasado en Recipientes Herméticos de Vidrio Borosilicato]
    D --> E[Refrigeración Estricta a 2°C - 4°C: 3 a 5 Días]
    D --> F[Ultracongelación a -18°C: 90 Días]
    
    E --> G[Regeneración Térmica: Centro Térmico > 74°C]
    F --> H[Descongelación Segura en Frío 24h a 4°C]
    H --> G
```
