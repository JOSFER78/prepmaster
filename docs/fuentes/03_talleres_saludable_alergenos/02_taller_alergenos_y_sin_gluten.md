...
```mermaid
graph TD
    A[Gestión de Alérgenos & Cocina Sin Gluten] --> B[Reglamento UE 1169/2011: 14 Alérgenos]
    A --> C[Diagnóstico Clínico Diferencial]
    A --> D[Protocolo APPCC Anti-Contaminación Cruzada]
    A --> E[Reología & Masterclass Sin Gluten]
    A --> F[Recetario Hipoalergénico de Rendimiento]
    
    B --> B1[Epítopos, Dosis Umbral & Alérgenos Ocultos]
    C --> C1[Alergia IgE vs Intolerancia Metabólica vs Celiaquía Autoinmune]
    D --> D1[Zonificación, Aerosoles de Harina 48h & Trazas <20 ppm]
    E --> E1[Matriz de Harinas, Psyllium Husk, Goma Xantana & Hidrogeles]
    F --> F1[4 Fórmulas Maestras con Control de Puntos Críticos PCC]
```
...
```mermaid
graph TD
    A[Flujo de Producción Libre de Alérgenos] --> B[Zonificación Temporal: Elaboración en Turno Inicial]
    A --> C[Aislamiento Atmosférico: Suspensión de Harinas 48h]
    A --> D[Código de Color Utensilios: Tablas & Cuchillos Morados]
    A --> E[Higienización Química: Desnaturalización con Detergente Alcalino]
    A --> F[Envasado Hermético & Rotulación Indeleble]
```
...
```mermaid
flowchart TD
    A[Arquitectura Mix Sin Gluten] --> B[Harinas Estructurales Base 40-50%]
    A --> C[Almidones Puros de Gelatinización 30-40%]
    A --> D[Harinas de Fuerza & Sabor 15-20%]
    A --> E[Red Hidrocoloide Aglutinante 2-4%]
    
    B --> B1[Arroz Blanco & Integral, Maíz Blanco, Avena GF]
    C --> C1[Almidón de Tapioca/Yuca, Maicena, Fécula de Patata]
    D --> D1[Trigo Sarraceno, Teff, Quinoa, Harina de Almendra]
    E --> E1[Psyllium Husk + Goma Xantana]
```
...
```mermaid
graph TD
    A[Agua Tibia 38°C + Levadura Seca + Miel Cruda] --> B[Activación 10 min: Formación de Espuma]
    C[Psyllium Husk en Polvo] --> D[Gelificación en Agua: Formación de Hidrogel 5 min]
    
    E[Harina Trigo Sarraceno + Arroz Integral + Almidón Tapioca + Maicena + Xantana + Sal] --> F[Tamizado en Seco]
    
    B --> G[Amasado Planetario / Manual: Harinas + Hidrogel Psyllium + AOVE]
    D --> G
    F --> G
    
    G --> H[Boleado Suave + Fermentación en Banneton 2h 30 min]
    H --> I[Greñado Longitudinal con Cuchilla]
    I --> J[Horneado en Dutch Oven Tapado 240°C 30 min -> Destapado 200°C 20 min]
    J --> K[Enfriado Obligatorio en Rejilla 3h antes de Cortar]
```
...
```mermaid
graph TD
    A[Patatas Monalisa Asadas con Piel al Horno] --> B[Pelado en Caliente + Pasapurés Fino]
    C[Almidón de Tapioca + Harina de Arroz Fina + Sal] --> D[Mezcla Seca Tamizada]
    
    B --> E[Unión sin Amasado Excesivo: Puré Seco + Harinas + AOVE]
    D --> E
    
    E --> F[Cilindros de Masa de 1.5 cm -> Corte en Dados y Marcado con Tenedor]
    F --> G[Hervido en Agua Salada Hirviendo: Flotan a la Superficie en 90 seg]
    
    H[Albahaca Fresca + Semillas de Calabaza Tostadas + AOVE + Levadura Nutricional + Ajo] --> I[Pesto Verde Hipoalergénico en Mortero/Trituradora]
    
    G --> J[Emulsión de Gnocchis con Pesto y Agua de Cocción]
    I --> J
```
...
```mermaid
graph TD
    A[Harina Trigo Sarraceno + Harina Arroz + Almidón Tapioca + Sal] --> B[Mezcla Seca Tamizada]
    C[AOVE Frío + Agua Helada] --> D[Emulsión Líquida]
    
    B --> E[Formado de Masa Quebrada *Pâte Brisée* Sin Gluten]
    D --> E
    
    E --> F[Estirado entre Papeles Sulfurizados + Forrado de Molde 26 cm]
    F --> G[Cocción en Blanco a 180°C 12 min con Peso de Legumbres]
    
    H[Puerros Pochados en AOVE + Dados de Salmón Fresco] --> I[Relleno Sólido]
    J[Huevos Camperos + Crema Avena GF + Nuez Moscada + Levadura Nutricional] --> K[Aparejo Líquido *Royale* Sin Lácteos]
    
    G --> L[Montaje: Base Precocida + Puerros & Salmón + Royale Líquida]
    I --> L
    K --> L
    L --> M[Horneado Final a 180°C 25-28 min hasta Cuajado y Dorado]
```
...
```mermaid
graph TD
    A[Manzanas Reineta en Cubos + Canela + Agua 30ml] --> B[Cocción Lenta 15 min: Compota Casera Rica en Pectina]
    C[Semillas de Lino Dorado Molidas + Agua Tibia] --> D[Hidratación 10 min: *Flax-Egg* Ligante]
    
    E[Harina de Almendra Extrafina + Cacao Puro 100% + Harina Arroz + Almidón Tapioca + Impulsor GF] --> F[Tamizado en Seco]
    
    B --> G[Integración Húmeda: Compota Triturada + *Flax-Egg* + AOVE Suave + Pasta de Dátiles]
    D --> G
    
    F --> H[Unión de Secos y Húmedos con Espátula: Masa Densa y Brillante]
    G --> H
    H --> I[Molde 22 cm con Papel Horno -> Horneado a 175°C por 32-35 min]
    I --> J[Enfriado Total en Rejilla + Cobertura de Frambuesas Frescas]
```
...
```mermaid
graph TD
    A[Recepción de Materias Primas con Ficha Técnica] --> B[Almacenamiento en Contenedores Herméticos con Etiqueta Morada]
    B --> C[Limpieza Profunda POES >65°C con Papel Azul Desechable]
    C --> D[Zonificación de Utensilios Morados y Desconexión de Ventilación]
    D --> E[Elaboración de Lotes Libres de Alérgenos]
    E --> F[Test Rápido de Tiras Inmunocromatográficas de Gluten <20 ppm]
    F --> G[Envasado Hermético con Ficha de Alérgenos Sellada]
```
