...
```mermaid
graph LR
    A[Bolsa TouChef en Nevera] --> B[Paso 1: Templar en Agua Caliente 5 min]
    B --> C[Paso 2: Marcado Express Maillard 45 seg]
...
```mermaid
graph TD
    subgraph Semáforo de Consumo
        G[🟢 DÍAS 1 A 3: Consumo Prioritario]
        Y[🟡 DÍAS 4 A 6: Consumo Secundario]
        R[🔴 DÍA 7+ : Consumo Final o Congelación]
    end
```
...
```mermaid
flowchart TD
    A[Bolsa en Congelador -18°C] --> B{¿Tienes tiempo?}
    B -- Sí: Planificado --> C[Pasar a Nevera 12h antes -> Regenerar según Ficha]
    B -- No: Inmediato --> D[Sumergir bolsa congelada en Agua Caliente 12-15 min -> Marcado]
```
